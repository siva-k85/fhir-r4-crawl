...
**Subfolders**:
- `markdown_rebuilt/` - Canonical set of 55 validated markdown files rebuilt from the official download mirror (average ~120KB)
  - 10 shortlist pages (Patient, Coverage, ExplanationOfBenefit, etc.)
  - 45 auxiliary pages (index, modules, foundation, security, etc.)
- `markdown/` - Legacy mixed-source extraction retained for audit purposes
- `README.md` - Output reference with file listing, validation notes, and rebuild instructions
...
cat 03_OUTPUTS_COMPLETE/markdown_rebuilt/hl7_org_fhir_R4_patient.md

# Coverage resource documentation
cat 03_OUTPUTS_COMPLETE/markdown_rebuilt/hl7_org_fhir_R4_coverage.md
```
...
1. **Web Crawl CAPTCHA Blocking**: HL7.org bot detection blocked direct page requests despite comprehensive anti-detection measures, causing redirected HTML (home page) to be saved. **Remediation**: Pivoted to the official `fhir-spec.zip` download for all shortlist pages and later rebuilt every markdown file via `scripts/10_rebuild_markdown_from_local.py` using the local mirror (plus a manually downloaded `searchform.html`).
