...
##  3.1.1 Search [](search.html#3.1.1 "link to here")
...
###  3.1.1.1 Summary Table [](search.html#Summary "link to here")
...
###  3.1.1.2 Introduction [](search.html#Introduction "link to here")
...
###  3.1.1.3 Handling Errors [](search.html#errors "link to here")
...
###  3.1.1.4 Standard Parameters [](search.html#standard "link to here")
####  3.1.1.4.1 Parameters for all resources [](search.html#all "link to here")
The following parameters apply [to all resources](resource.html#search): `_content`, `_id`, `_lastUpdated`, `_profile`, `_query`, `_security`, `_source`, `_tag`. In addition, the search parameter [`_text`](#text) and [`_filter`](#filter), (documented below) also applies to all resources (as do the search result parameters). 
...
####  3.1.1.4.2 Parameters for each resource [](search.html#parameters "link to here")
...
####  3.1.1.4.3 Search Parameter Types [](search.html#ptypes "link to here")
...
####  3.1.1.4.4 Modifiers [](search.html#modifiers "link to here")
Parameters are defined per resource. Parameter names may specify a modifier as a suffix. The modifiers are separated from the parameter name by a colon. Modifiers are:
  ...
  * For string: `:exact` returns results that match the entire supplied parameter, including casing and combining characters. Note that the handling of (extended) [grapheme clusters ![](external.png)](http://unicode.org/reports/tr29/#Grapheme_Cluster_Boundaries) is within the discretion of the server, i.e. the server decides if a string search parameter matches on canonically equivalent characters, or matches on the actual used Unicode code points, or `:contains` (case insensitive and combining character-insensitive, search text matched anywhere in the string), instead of the default behavior (case insensitive and accent-insensitive, partial matches at the start of the string).
...
####  3.1.1.4.5 Prefixes [](search.html#prefix "link to here")
...
####  3.1.1.4.6 number [](search.html#number "link to here")
...
####  3.1.1.4.7 date [](search.html#date "link to here")
...
####  3.1.1.4.8 string [](search.html#string "link to here")
...
####  3.1.1.4.9 uri [](search.html#uri "link to here")
The uri parameter refers to an element that contains a URI ([RFC 3986 ![](external.png)](https://tools.ietf.org/html/rfc3986)). By default, matches are precise (e.g. case, accent, and escape) sensitive, and the entire URI must match. The modifier `:above` or `:below` can be used to indicate that partial matching is used. For example: 
```
...
####  3.1.1.4.10 token [](search.html#token "link to here")
...
For tokens, matches are literal (e.g. not based on [subsumption](codesystem.html#subsumption) or other code system features). Match is case sensitive unless the underlying semantics for the context indicate that the token should be interpreted case-insensitively (see, e.g. [CodeSystem.caseSensitive](codesystem-definitions.html#CodeSystem.caseSensitive)). Note that matches on `_id` are always case sensitive. If the underlying data type is `string[](datatypes.html#string)` then the search is not case sensitive. 
...
#####  3.1.1.4.10.1 Searching Mime Types [](search.html#mimetype "link to here")
...
####  3.1.1.4.11 quantity [](search.html#quantity "link to here")
...
Specifying a system and a code for the search implies that the search is based on a particular code system - usually [UCUM ![](external.png)](http://unitsofmeasure.org), and that a precise (and potentially canonical) match is desired. In this case, it is inappropriate to search on the human display for the unit, which can be is uncontrolled and may unpredictable. 
The search processor may choose to perform a search based on [canonical units](datatypes.html#quantity) (e.g. any value where the units can be converted to a value in mg in the case above). For example, an observation may have a value of `23 mm/hr`. This is equal to `0.23 m/hr`. The search processer can choose to normalise all the values to a canonical unit such as `6.4e-6 m/sec`, and convert search terms to the same units (m/sec). Such conversions can be performed based on the semantics defined in [UCUM ![](external.png)](http://unitsofmeasure.org)
####  3.1.1.4.12 reference [](search.html#reference "link to here")
A reference parameter refers to [references between resources](references.html). For example, find all Conditions where the subject reference is a particular patient, where the patient is selected by name or identifier. The interpretation of a _reference_ parameter is either: 
...
####  3.1.1.4.13 References and Versions [](search.html#versions "link to here")
...
####  3.1.1.4.14 Searching Hierarchies [](search.html#recursive "link to here")
...
####  3.1.1.4.16 Reverse Chaining [](search.html#has "link to here")
...
####  3.1.1.4.17 Composite Search Parameters [](search.html#combining "link to here")
...
####  3.1.1.4.18 Handling Missing Data [](search.html#missing "link to here")
...
####  3.1.1.4.19 Escaping Search Parameters [](search.html#escaping "link to here")
...
####  3.1.1.4.20 Text Search Parameters [](search.html#content "link to here")
...
Implementers could consider using the rules specified by the [ OData specification for the $`search` parameter ![](external.png)](http://docs.oasis-open.org/odata/odata/v4.0/cs01/part1-protocol/odata-v4.0-cs01-part1-protocol.html#_The_%24search_System). Typical implementations would use Lucene, Solr, an SQL-based full text search, or some similar indexing service. 
####  3.1.1.4.21 Special Parameters [](search.html#special "link to here")
...
####  3.1.1.4.22 Searching by list [](search.html#list "link to here")
...
####  3.1.1.4.23 Advanced filtering [](search.html#_filter "link to here")
...
####  3.1.1.4.24 Specifying the type of Resource [](search.html#_type "link to here")
...
###  3.1.1.5 Managing Returned Resources [](search.html#return "link to here")
####  3.1.1.5.1 Sorting [](search.html#_sort "link to here")
...
The _total parameter has the status [trial-use](versions.html#std-process "Trial Content") pending real world experience of it's use. 
####  3.1.1.5.2 Total number of matching resources [](search.html#total "link to here")
...
####  3.1.1.5.3 Page Count [](search.html#count "link to here")
...
####  3.1.1.5.4 Including other resources in result (`_include` and `_revinclude`) [](search.html#revinclude "link to here")
...
####  3.1.1.5.5 Contained Resources [](search.html#containedType "link to here")
...
####  3.1.1.5.6 External References [](search.html#external "link to here")
...
####  3.1.1.5.7 Paging [](search.html#Paging "link to here")
...
####  3.1.1.5.8 Summary [](search.html#summary "link to here")
...
####  3.1.1.5.9 Elements [](search.html#elements "link to here")
...
####  3.1.1.5.10 Relevance [](search.html#score "link to here")
...
###  3.1.1.6 Server Conformance [](search.html#conformance "link to here")
...
* * *
###  3.1.1.7 Advanced Search [](search.html#query "link to here")
...
###  3.1.1.8 Search Result Currency [](search.html#currency "link to here")
...
###  3.1.1.9 Summary Tables [](search.html#table "link to here")
...
[`_count`](#count) | [number ](#number%20) | Number of results per page | Whole number  
