...
**This page is part of a downloaded copy of this specification.** This page is part of the FHIR Specification (v4.0.1: R4 - Mixed [Normative](https://confluence.hl7.org/display/HL7/HL7+Balloting "Normative Standard") and [STU](https://confluence.hl7.org/display/HL7/HL7+Balloting "Standard for Trial-Use")). For a full list of available versions, see the [Directory of published versions ![](external.png)](http://hl7.org/fhir/directory.html)
  * [Using Codes](#)
  * [Code Systems](terminologies-systems.html)
...
#  4.1 Using Codes in Resources [](terminologies.html#4.1 "link to here")
...
Many elements in the FHIR resources have a **coded value** : some fixed string (a sequence of characters) assigned elsewhere that identifies some defined "concept". The sequence of characters and its meaning may be defined in one of several places: 
  ...
  * An HL7 specification ([HL7 v3 ![](external.png)](https://www.hl7.org/implement/standards/product_brief.cfm?product_id=186) code system, or [HL7 v2 ![](external.png)](http://www.hl7.org/implement/standards/product_brief.cfm?product_id=185) table)
  * Some external terminology or ontology such as [LOINC ![](external.png)](http://loinc.org), or [SNOMED CT ![](external.png)](http://www.snomed.org)
...
The FHIR framework for using coded values is based on the fundamental framework defined in section 5 of the [HL7 v3 Core Principles ![](external.png)](http://www.hl7.org/documentcenter/public/standards/V3/core_principles/infrastructure/coreprinciples/v3modelcoreprinciples.html) document, including the separation between code systems and value sets. 
...
---  
[code](datatypes.html#code) | The instance represents the _code_ only. The _system_ is implicit - it is defined as part of the definition of the element, and not carried in the instance.   
[Coding](datatypes.html#Coding) | A data type that has a _code_ and a _system_ element that identifies where the definition of the code comes from  
[CodeableConcept](datatypes.html#CodeableConcept) | A type that represents a concept by plain text and/or one or more `coding` elements (See the [datatype notes](datatypes.html#CodeableConcept) for a discussion of code translations and using text in CodeableConcept)  
...
[Quantity](datatypes.html#Quantity) | The instance has _system_ and _code_ elements for carrying a code for the type of unit, and these can be bound to a value set  
[string](datatypes.html#string) | The instance carries a string. In some cases, applications may wish to control the set of valid strings for a particular element, so the string value can be treated as a coded element (like `code`)  
[uri](datatypes.html#uri) | Like `string`, URIs can be treated as a coded element  
Notes: 
  * Generally, the choice of data type is dictated by the resource itself. When choosing a data type for an [extension](extensibility.html), see the FHIR Confluence pages for [advice about data type choice ![](external.png)](https://confluence.hl7.org/display/FHIR/Guide+to+Designing+Resources#Choice+of+coding+data+type).
...
##  4.1.1 Coded Values, Code Systems, and Value Sets [](terminologies.html#valuesets "link to here")
...
##  4.1.2 Choosing a system [](terminologies.html#system "link to here")
...
The correct value to use in the _system_ for a given code system can be determined by working through the following list, in order: 
  ...
  * The FHIR [community code system registry ![](external.png)](https://registry.fhir.org/search?q=resourcetype:codesystem) - if a code system is listed here with [status = active](namingsystem-definitions.html#NamingSystem.status), it SHALL be used
  * An OID registered in the [HL7 OID registry ![](external.png)](http://hl7.org/oid) - if a code system is registered here, the OID SHOULD be used (using the syntax urn:oid:[oid])


If a code system is not resolved by this list, and there is no publisher to consult, implementers must choose a URI to use. The priority should be to choose a unique value that won't accidentally be used by another implementer for a different purpose - or a very similar purpose with a different scope. See [Registering Code Systems ![](external.png)](https://confluence.hl7.org/display/FHIR/Registering+Code+Systems) for further advice. 
...
Note: if the code system is made available packaged inside a [ValueSet](valueset.html) resource, the correct URL for the `system` value is _ValueSet.codeSystem.system_ , not _ValueSet.uri_. 
##  4.1.3 Complex Code Systems [](terminologies.html#grammar "link to here")
...
Some code systems define rules for how complex expressions can be built using the basic concepts defined by the code system. This is sometimes referred to as "post-coordination". Some of the more notable code systems that define grammars for expressions are: 
  ...
  * [UCUM](ucum.html)
  * [Mime Types ![](external.png)](http://tools.ietf.org/html/bcp13)
  * [Language ![](external.png)](http://tools.ietf.org/html/bcp47)
  * [ICD-[X]](icd.html)
...
##  4.1.4 Controlling the use of Coded Values [](terminologies.html#binding "link to here")
...
###  4.1.4.1 Value Set References [](terminologies.html#references "link to here")
...
####  4.1.4.1.1 Direct Value Set references [](terminologies.html#direct "link to here")
...
####  4.1.4.1.2 Logical Value Set references [](terminologies.html#logical "link to here")
...
####  4.1.4.1.3 Unbound [](terminologies.html#unbound "link to here")
...
##  4.1.5 Binding Strengths [](terminologies.html#strength "link to here")
...
###  4.1.5.1 Required [](terminologies.html#required "link to here")
_To be conformant, codes in this element SHALL be from the specified value set._
In the standard, this is generally used for elements where the value needs to be strictly controlled so that everyone can interpret it with confidence. Generally, this is used for elements with type [code](datatypes.html#code): 
  ...
  * the element is bound to some external standard that defines the set of valid codes that can be used (typical examples are [Mime Types ![](external.png)](http://www.rfc-editor.org/bcp/bcp13.txt), [Language Codes ![](external.png)](http://tools.ietf.org/html/bcp47), [UCUM ![](external.png)](http://unitsofmeasure.org), etc.)
...
###  4.1.5.2 Extensible [](terminologies.html#extensible "link to here")
_To be conformant, codes in this element SHALL be from the specified value set if any of the codes within the value set can apply to the concept being communicated._
...
####  4.1.5.2.1 Examples for Extensible Bindings [](terminologies.html#4.1.5.2.1 "link to here")
...
###  4.1.5.3 Preferred [](terminologies.html#preferred "link to here")
_Instances are encouraged to draw from the specified codes for interoperability purposes but are not required to do so to be considered conformant._
...
####  4.1.5.3.1 Examples for Preferred Bindings [](terminologies.html#4.1.5.3.1 "link to here")
See [examples](terminologies-binding-examples.html#preferred) of how Preferred bindings work. 
####  4.1.5.3.2 Example Bindings [](terminologies.html#example "link to here")
_Instances are not expected or even encouraged to draw from the specified value set. The value set merely provides examples of the types of concepts intended to be included._
Example bindings are used when an element has a very broad meaning (such as [List](list.html).code), or there is no consensus over the correct codes to be used. For these bindings: 
  * **Coding** : the _system_ /_code_ values MAY be one of the codes in the value set. 
  * **CodeableConcept** : one of the _coding_ elements MAY contain a _system_ /_code_ that is in the value set.
...
##  4.1.6 Other notes [](terminologies.html#other "link to here")
...
###  4.1.6.1 Binding String Values [](terminologies.html#strings "link to here")
...
###  4.1.6.2 Terminology Service [](terminologies.html#Terminology-Service "link to here")
