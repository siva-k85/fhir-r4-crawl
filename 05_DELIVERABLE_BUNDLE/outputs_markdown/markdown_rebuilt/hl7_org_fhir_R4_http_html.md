...
##  3.1.0 RESTful API [](http.html#3.1.0 "link to here")
...
###  3.1.0.1 General Considerations [](http.html#general "link to here")
...
####  3.1.0.1.1 Style Guide [](http.html#styleguide "link to here")
...
Implementations constructing URLs using these patterns SHOULD conform to [RFC 3986 Section 6 Appendix A ![](external.png)](https://tools.ietf.org/html/rfc3986#appendix-A) which requires percent-encoding for a number of characters that occasionally appear in the URLs (mainly in search parameters). 
...
####  3.1.0.1.2 Service Base URL [](http.html#general "link to here")
...
Note: All URLs (and ids that form part of the URL) defined by this specification are case sensitive. Clients SHOULD encode URLs using UTF-8, and servers SHOULD decode them assuming they are UTF-8 (for background, [see here ![](external.png)](http://stackoverflow.com/questions/912811/what-is-the-proper-way-to-url-encode-unicode-characters)). 
...
####  3.1.0.1.3 Resource Metadata and Versioning [](http.html#versioning "link to here")
...
####  3.1.0.1.4 Security [](http.html#3.1.0.1.4 "link to here")
Using HTTPS is optional, but all production exchange of healthcare data SHOULD use SSL and additional security as appropriate. See [HTTP Security](security.html#http) for further information. Most operations will require user authentication, and all operations that do so are subject to [RBAC ![](external.png)](https://en.wikipedia.org/wiki/Role-based_access_control) and/or [ABAC ![](external.png)](https://en.wikipedia.org/wiki/Attribute-based_access_control), and some operations may depend on appropriate consent being granted. 
...
####  3.1.0.1.5 HTTP Status Codes [](http.html#Status-Codes "link to here")
...
####  3.1.0.1.6 Managing Return Content [](http.html#return "link to here")
...
####  3.1.0.1.7 conditional read [](http.html#cread "link to here")
...
####  3.1.0.1.8 create/update/patch/transaction [](http.html#ops "link to here")
...
The client can indicate whether the entire resource is returned using the [HTTP return preference ![](external.png)](https://tools.ietf.org/html/rfc7240#section-4.2): 
```
...
####  3.1.0.1.9 Content Types and encodings [](http.html#mime-type "link to here")
...
Servers SHALL support server-driven content negotiation as described in [section 12 ![](external.png)](https://tools.ietf.org/html/rfc7231#section-3.4) of the HTTP specification. 
...
####  3.1.0.1.10 FHIR Version Parameter [](http.html#version-parameter "link to here")
...
[FHIR R1 ![](external.png)](http://hl7.org/fhir/DSTU1) (DSTU 1) | 0.0  
---|---  
[FHIR R2 ![](external.png)](http://hl7.org/fhir/DSTU2) (DSTU 2) | 1.0  
[FHIR R3 ![](external.png)](http://hl7.org/fhir/STU3) (STU3, or just R3) | 3.0  
[FHIR R4 ![](external.png)](http://hl7.org/fhir/r4) (this version) | 4.0 (once published)  
Intermediate balloted releases may also be encountered occasionally - see [publications directory ![](external.png)](http://hl7.org/fhir/directory.html). Versions from before the publication of the [first DSTU ![](external.png)](http://hl7.org/fhir/DSTU1/index.html) (which is 0.0) are not supported. 
...
####  3.1.0.1.11 General parameters [](http.html#parameters "link to here")
...
####  3.1.0.1.12 Support for Versions [](http.html#versions "link to here")
...
###  3.1.0.2 read [](http.html#read "link to here")
...
###  3.1.0.3 vread [](http.html#vread "link to here")
...
A HEAD request can also be used - [see below](#head). 
###  3.1.0.4 update [](http.html#update "link to here")
...
####  3.1.0.4.1 Update as Create [](http.html#upsert "link to here")
...
####  3.1.0.4.2 Rejecting Updates [](http.html#rejecting-updates "link to here")
...
####  3.1.0.4.3 Conditional update [](http.html#cond-update "link to here")
...
###  3.1.0.5 Managing Resource Contention [](http.html#concurrency "link to here")
[Lost Updates ![](external.png)](http://www.w3.org/1999/04/Editing/), where two clients update the same resource, and the second overwrites the updates of the first, can be prevented using a combination of the [ETag ![](external.png)](https://tools.ietf.org/html/rfc7232#section-2.3) and [If-Match ![](external.png)](https://tools.ietf.org/html/rfc7232#section-3.1) header. This is also known as 'Optimistic Locking'. 
...
###  3.1.0.6 patch [](http.html#patch "link to here")
...
For PATCH Examples, see [the FHIR test cases](http://hl7.org/fhir/R4/test-cases.zip). 
...
###  3.1.0.7 delete [](http.html#delete "link to here")
...
####  3.1.0.7.1 Conditional delete [](http.html#3.1.0.7.1 "link to here")
Unlike this rest of this page, the conditional create, update, patch and delete are trial use until further experience is gained with their use. Their status will be reviewed in a future version of FHIR. 
...
Note that transactions and conditional create/update/delete are complex interactions and it is not expected that every server will implement them. Servers that don't support the conditional update SHOULD return an HTTP `400` error and MAY include an [OperationOutcome](operationoutcome.html). 
###  3.1.0.8 create [](http.html#create "link to here")
...
For additional information on how systems may behave when processing updates, refer to the [Variations between Submitted data and Retrieved data](updates.html) page. 
####  3.1.0.8.1 Conditional create [](http.html#ccreate "link to here")
...
Note that transactions and conditional create/update/delete are complex interactions and it is not expected that every server will implement them. Servers that don't support the conditional update SHOULD return an HTTP `400` error and MAY include an [OperationOutcome](operationoutcome.html). 
###  3.1.0.9 search [](http.html#search "link to here")
...
Because of the way that some user agents and proxies treat `GET` and `POST` requests, in addition to the get based search method above, servers that support _search_ SHALL also support a `POST` based search: 
```
...
All these search interactions take a series of parameters that are a series of `name=value` pairs encoded in the URL (or as an `application/x-www-form-urlencoded` ([Specification ![](external.png)](https://url.spec.whatwg.org/#application/x-www-form-urlencoded)) submission for a `POST`). (See [W3C HTML forms ![](external.png)](http://www.w3.org/TR/REC-html40/interact/forms.html#form-content-type)). 
...
The result collection can be long, so servers may use paging. If they do, they SHALL use the method [described below](#paging) (adapted from [RFC 5005 (Feed Paging and Archiving ![](external.png)](https://tools.ietf.org/html/rfc5005)) for breaking the collection into pages if appropriate. The server MAY also return an [OperationOutcome](operationoutcome.html) resource within the `searchset` Bundle entries that contains additional information about the search; if one is sent it SHALL NOT include any issues with a `fatal` or `error` [severity](valueset-issue-severity.html), and it SHALL be marked with a [Bundle.entry.search.mode](valueset-search-entry-mode.html) of `outcome`. 
...
####  3.1.0.9.1 Variant Searches [](http.html#vsearch "link to here")
...
###  3.1.0.10 capabilities [](http.html#capabilities "link to here")
...
###  3.1.0.11 batch/transaction [](http.html#transaction "link to here")
...
The `transaction` mode is especially useful where one would otherwise need multiple interactions, possibly with a risk of loss of referential integrity if a later interaction fails (e.g. when storing a Provenance resource and its corresponding target resource, or and [IHE-MHD transaction "Provide Document Resources" ![](external.png)](https://wiki.ihe.net/index.php/MHD_Implementation_Guide) with a DocumentManifest, and some number of DocumentReference, List, and Binary resources). 
...
####  3.1.0.11.1 Batch Processing Rules [](http.html#brules "link to here")
...
####  3.1.0.11.2 Transaction Processing Rules [](http.html#trules "link to here")
...
####  3.1.0.11.3 Batch/Transaction Response [](http.html#transaction-response "link to here")
...
####  3.1.0.11.4 Accepting other Bundle types [](http.html#other-bundles "link to here")
...
###  3.1.0.12 history [](http.html#history "link to here")
...
###  3.1.0.13 Transactional Integrity [](http.html#transactional-integrity "link to here")
...
####  3.1.0.13.1 Conformance [](http.html#conformance-rules "link to here")
...
###  3.1.0.14 Paging [](http.html#paging "link to here")
Servers SHOULD support paging for the results of a [search](#search) or [history](#history) interaction, and if they do, they SHALL conform to this method (adapted from [RFC 5005 (Feed Paging and Archiving) ![](external.png)](https://tools.ietf.org/html/rfc5005) for sending continuation links to the client when returning a [Bundle](bundle.html) (e.g. with `history` and `search`). If the server does not do this then there is no way to continue paging. 
...
###  3.1.0.15 Support for HEAD [](http.html#head "link to here")
...
###  3.1.0.16 Custom Headers [](http.html#custom "link to here")
...
###  3.1.0.17 Summary [](http.html#summary "link to here")
These tables present a summary of the interactions described here. Note that _all_ requests may include an optional `Accept` header to indicate the format used for the response (this is even true for `DELETE` since an OperationOutcome may be returned). 
