---
url: https://hl7.org/fhir/R4/security.html
title: security.html
source: official_download
extracted: local_file_conversion
mirror_path: security.html
---
...
#  6.1.0 FHIR Security [](security.html#6.1.0 "link to here")
[FHIR Infrastructure ![](external.png)](http://www.hl7.org/Special/committees/fiwg/index.cfm) Work Group |  [Maturity Level](versions.html#maturity): 4 |  [Standards Status](versions.html#std-process): [Trial Use](versions.html#std-process)  
---|---|---  
...
Time critical concerns regarding security flaws in the FHIR specification should be addressed to the [FHIR email list ![](external.png)](https://confluence.hl7.org/display/FHIR/Mailing+List+Instructions) for prompt consideration. 
##  6.1.0.1 General Considerations [](security.html#general "link to here")
...
![](security-layout.png) |  |  | ![](security-icon-user.png) | The consumer that is using a healthcare related system  
---|---  
![](security-icon-app.png) | The client application the user is using (application, mobile app, website, etc.)  
![](security-icon-sec.png) | The security system (authentication and access control)  
![](security-icon-fhir.png) | The clinical/healthcare repository  
...
##  6.1.0.2 Security and Privacy Considerations [](security.html#SecPrivConsiderations "link to here")
...
###  6.1.0.2.1 Anonymous READ Access Resources [](security.html#Anonymous "link to here")
...
###  6.1.0.2.2 Business Sensitive Resources [](security.html#Business "link to here")
...
###  6.1.0.2.3 Individual Sensitive [](security.html#Individual "link to here")
...
###  6.1.0.2.4 Patient Sensitive [](security.html#Patient "link to here")
...
###  6.1.0.2.5 Not classified [](security.html#Unknowable "link to here")
...
##  6.1.0.3 Communications [](security.html#http "link to here")
For the [RESTful API](http.html), normal HTTP security rules apply. Please follow the [ HTTP specification Security Considerations section 9 ![](external.png)](https://tools.ietf.org/html/rfc7231#section-9). The [Service Base URL](http.html#root) will specify whether TLS is required. Client authentication may be required by the server, possibly including the requirement for client certificates. When returning responses to non-authorized clients, ensure that Hypertext Transfer Protocol (HTTP) headers of a web server and API error messages or faults do not disclose detailed information about the underlying web server that could be the source of potential exploitation. 
Please follow IETF Best Current Practice ([BCP 195 ![](external.png)](https://tools.ietf.org/html/bcp195)) - ["Recommendations for Secure Use of Transport Layer Security (TLS) and Datagram Layer Security (DTLS)" ![](external.png)](https://tools.ietf.org/html/bcp195). This recommends TLS 1.2 or higher be used for all production data exchange, limits support for lower versions of TLS, and forbids SSL. Versions of TLS prior to TLS 1.2 should be used only where TLS 1.2 or higher is not available between partners. When using TLS, use with strong cipher suites (e.g. AES). 
...
When it is desirable to support browser-based javascript client applications, servers SHOULD consider enabling [cross-origin resource sharing (CORS) ![](external.png)](https://www.w3.org/TR/cors/) for the [REST operations](http.html). Consider advice from sources including [Enable-CORS ![](external.png)](http://enable-cors.org/) and [Moesif blog on Guide to CORS Pitfalls ![](external.png)](https://www.moesif.com/blog/technical/cors/Authoritative-Guide-to-CORS-Cross-Origin-Resource-Sharing-for-REST-APIs/). Experience shows that this is an area where ongoing issues may be expected as security holes are found and closed on an ongoing basis. 
##  6.1.0.4 Authentication [](security.html#authentication "link to here")
Other than testing systems, FHIR servers should authenticate the clients. The server may choose to authenticate the client system and trust it, or to authenticate the individual user by a variety of techniques. For web-centric environments it is recommended to use [OpenID Connect ![](external.png)](http://openid.net/connect/) (or other suitable authentication protocol) to verify identity of the end user, where it is necessary that end-users be identified to the application. It is recommended that [OAuth ![](external.png)](http://oauth.net/) be used to authenticate and/or authorize the client and user. The [Smart-On-FHIR ![](external.png)](http://docs.smarthealthit.org/) profile on OAuth is a recommended method for using OAuth. 
The [HEART Working Group ![](external.png)](https://openid.net/wg/heart/) has developed a set of privacy and security specifications that enable an individual to control the authorization of access to RESTful health-related data sharing APIs, and to facilitate the development of interoperable implementations of these specifications by others. IHE [IUA Profile ![](external.png)](http://wiki.ihe.net/index.php/Internet_User_Authorization) constrains OAuth token attributes to support Healthcare. 
...
##  6.1.0.5 Authorization/Access Control [](security.html#binding "link to here")
...
For one source of further information, see the [IHE Access Control white paper ![](external.png)](http://www.ihe.net/Technical_Framework/upload/IHE_ITI_TF_WhitePaper_AccessControl_2009-09-28.pdf)
...
###  6.1.0.5.1 Access Control Considerations [](security.html#6.1.0.5.1 "link to here")
...
###  6.1.0.5.2 Access Denied Response Handling [](security.html#AccessDenied "link to here")
...
##  6.1.0.6 Audit Logging [](security.html#audit "link to here")
...
When used to record security and privacy relevant events, the AuditEvent can then be used by properly authorized applications to support audit reporting, alerting, filtering, and forwarding. This model has been developed and used in healthcare for a decade as [IHE-ATNA profile ![](external.png)](http://wiki.ihe.net/index.php?title=Audit_Trail_and_Node_Authentication). ATNA log events can be automatically converted to AuditEvent resources, and from there, client applications are able to search the audit events, or subscribe to them. 
...
##  6.1.0.7 Attachments [](security.html#attachments "link to here")
