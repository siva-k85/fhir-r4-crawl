---
url: https://hl7.org/fhir/R4/terminology-service.html
title: terminology-service.html
source: official_download
extracted: local_file_conversion
mirror_path: terminology-service.html
---
...
##  4.7 Terminology Service [](terminology-service.html#4.7 "link to here")
...
###  4.7.1 Security [](terminology-service.html#security "link to here")
...
###  4.7.2 Basic Concepts [](terminology-service.html#concepts "link to here")
...
In addition, implementers should be familiar with the [operations framework](operations.html). Further useful information about terminologies may be found in: 
  * Underlying Principles: [HL7 v3 Core Principles ![](external.png)](http://www.hl7.org/documentcenter/public/standards/V3/core_principles/infrastructure/coreprinciples/v3modelcoreprinciples.html)
  * SNOMED CT [technical documentation ![](external.png)](http://ihtsdo.org/fileadmin/user_upload/doc/). Note: "namespace" is used differently here from the way it is used by IHTSDO (see [discussion here](terminologies.html))


####  4.7.2.1 External Code Systems [](terminology-service.html#externals "link to here")
...
####  4.7.2.2 Implementation Note [](terminology-service.html#impl1 "link to here")
...
> **Implementation Note:** _Implementers interested in working with existing published terminologies for which the CodeSystem infrastructure is not suitable should discuss their needs with HL7 to get the list above extended._
...
####  4.7.2.3 Operations across all value sets [](terminology-service.html#all "link to here")
...
####  4.7.2.4 Terminology Maintenance [](terminology-service.html#maint "link to here")
...
###  4.7.3 Value Set Expansion [](terminology-service.html#expand "link to here")
...
The server returns a value set that contains the current list of codes that meet the filter criteria (or an [OperationOutcome](operationoutcome.html) with an error if the expansion fails). Note that some value sets expand to many thousands of codes, or even an infinite number, and for these, the server SHOULD return an [error code _too-costly_](valueset-issue-type.html#too-costly). In these cases, the client can try again with a more specific text filter to reduce the number of codes returned - this may result in a valid expansion. 
...
###  4.7.4 Concept Lookup / Decomposition [](terminology-service.html#lookup "link to here")
...
####  4.7.4.1 Standard Properties [](terminology-service.html#standard-props "link to here")
...
lang.X | Designations in language X (where X is an IETF Language code, see [BCP-47 ![](external.png)](http://tools.ietf.org/html/bcp47)  
...
###  4.7.5 Value Set Validation [](terminology-service.html#validation "link to here")
...
###  4.7.6 Subsumption testing [](terminology-service.html#subsumes "link to here")
To test the subsumption relationship between _code/Coding A_ and _code/Coding B_ , perform a $subsumes operation. Subsumption testing is based on [the CodeSystem definition of subsumption](codesystem.html#subsumption). The client passes the server the following information: 
...
###  4.7.7 Batch Validation [](terminology-service.html#batch "link to here")
...
**Example**
A request to validate 2 concepts from a [CDA ![](external.png)](http://www.hl7.org/implement/standards/product_brief.cfm?product_id=7) document, with OIDs for value set identifiers: 
```
...
###  4.7.8 Translations [](terminology-service.html#translate "link to here")
A client can ask a server to translate a concept from one value set to another. Typically, this is used to translate between code systems (e.g. from LOINC to SNOMED CT, or from a [HL7 V3 ![](external.png)](https://www.hl7.org/implement/standards/product_brief.cfm?product_id=186) code to a [HL7 V2 ![](external.png)](http://www.hl7.org/implement/standards/product_brief.cfm?product_id=185) code). The client calls [the translate operation](conceptmap-operation-translate.html) and passes the following parameters: 
...
**Example**
Translate from FHIR Composition status to [HL7 v3 ![](external.png)](https://www.hl7.org/implement/standards/product_brief.cfm?product_id=186) Act Status (based on [this defined concept map](cm-composition-status-v3.html)): 
```
...
###  4.7.9 Batch Translation [](terminology-service.html#batch2 "link to here")
...
**Example**
A request to translate 2 concepts from a [CDA ![](external.png)](http://www.hl7.org/implement/standards/product_brief.cfm?product_id=7) document, with OIDs for value set identifiers: 
```
...
###  4.7.10 Maintaining a Closure Table [](terminology-service.html#closure "link to here")
...
An alternative approach is to generate a [transitive closure table ![](external.png)](https://en.wikipedia.org/wiki/Transitive_closure#In_graph_theory) which lists all the possible transitive subsumption relationships, and [allows for rapid execution of these kind of queries ![](external.png)](http://karwin.blogspot.com.au/2010/03/rendering-trees-with-closure-tables.html) . However, this has other problems: 
...
####  4.7.10.1 Initializing a Closure Table [](terminology-service.html#closure-init "link to here")
...
####  4.7.10.2 Adding to a Closure Table [](terminology-service.html#closure-add "link to here")
...
####  4.7.10.3 Re-running Closure operation [](terminology-service.html#closure-rerun "link to here")
...
####  4.7.10.4 Making use of the Closure Table [](terminology-service.html#closure-use "link to here")
